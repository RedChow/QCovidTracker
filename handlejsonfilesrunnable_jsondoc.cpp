...
#include "handlejsonfilesrunnable_jsondoc.h"
#include <QSqlQuery>
#include <QSqlError>
#include <QVariant>
#include <QThread>
...
HandleJsonFilesRunnableJSON::HandleJsonFilesRunnableJSON()
{
...
QSqlDatabase HandleJsonFilesRunnableJSON::db() {
...
void HandleJsonFilesRunnableJSON::setJsonDirectory(QString dir) {
    jsonDirectory = dir;
}

void HandleJsonFilesRunnableJSON::setDbName(QString p_dbName) {
    dbName = p_dbName;
}

void HandleJsonFilesRunnableJSON::setJsonFileList(QStringList jsonFiles) {
    jsonFileList = jsonFiles;
}

void HandleJsonFilesRunnableJSON::setFileName(QString p_FileName) {
    fileName = p_FileName;
}

void HandleJsonFilesRunnableJSON::setBeginningAndEnd(int b, int e) {
    beginning = b;
    end = e;
}

void HandleJsonFilesRunnableJSON::run() {
    int start{0};
    QJsonObject json;
    int stateId{-1};
    QSqlQuery query(db());
    ...
    for (start = beginning; start < end; start++) {
        ...
        /*
         * Only need stateID and d_date to insert into covid_history_json
